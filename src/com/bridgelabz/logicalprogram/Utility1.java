...
public class Utility1 {
	...
	public static void readFileInteger() throws FileNotFoundException {

		int arr[] = new int[10];
		FileReader file = new FileReader("/home/bridgeit/Desktop/IntegerFile.text");
...
//***************flip coin**************************8
	...
	public double[] isFlip(int n) {

		double head = 0, tail = 0, perh, pert;
		double v = 0;
		...
		perh = (double) (head / 5) * 100;
		pert = (double) (tail / 5) * 100;

		double[] arr = { head, tail, perh, pert };
...
//Function to check given no is amsrong or not 
	public int isAmstrong(int n) {

		int rem = 0, sum = 0, i = 0;

//eg 153 this are amstrong no.s
		...
		while (n > 0) {
			...
			sum = sum + (rem * rem * rem); // 3*3*3+5*5*5+1*1*1
			n = n / 10; // 15 , 1
...
//Function to check given string are anagram or not
//@param are two string to be checked

	public int isAnagram(String str1, String str2) {
		str1 = str1.toLowerCase();
		str2 = str2.toLowerCase();

		if (str1.length() == str2.length())// if length is not same they are not anagram
		{
...
//***********insertElementInArray******************************
	...
	public int[] insertEleInArray(int ele, int[] arr, int size, int pos) {

//{5,4,6,10} insert 7 at pos 2 means between 5 and 4
...
//************gamblor**************************************
	...
	public int gambler(int cash, int trial, int goal) {

		int bet = 0, win = 0;

		for (int i = 0; i <= trial; i++) {

			while (trial > 0 && goal > cash) {
				bet++;
...
//***************stop watch************
	...
	public Double stopWatch(long startTime, long stopTime) {
		...
		double result = (stopTime - startTime) / 1000;
		return result;
...
//*********************Descending order******************
//Function to retun descending order of array

	public int[] DescendingOrderOfArray(int[] arr) {
...
//****************Linear search*****************************
	...
	public int linearSearch(int a[], int x, int item) {
